...
public class TC_03 {
    ...
    @Test
    public void navigateToKuwait() {
        var openKuwaitPage = new landingPage(driver).checkFooter().OpenKuwaitPage();
    }
    @Test(dependsOnMethods = "navigateToKuwait")
    ...
    public void validateCurrency() {
        ...
        Validations.assertThat().element(driver, liteCurrency).text().contains(TestData.getTestData("LitePackage.KwuaitCurrency")).perform();

        var classicCurrency = new CountryPage(driver).getClassicCurrency();
        Validations.assertThat().element(driver, classicCurrency).text().contains(TestData.getTestData("ClassicPackage.KwuaitCurrency")).perform();

        var premiumCurrency = new CountryPage(driver).getPremiumCurrency();
        Validations.assertThat().element(driver, premiumCurrency).text().contains(TestData.getTestData("PremiumPackage.KwuaitCurrency")).perform();
    }
